                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">
                                    {{ ucfirst($order->user ? $order->user->name : 'Unknown User') }}</div>
                            </td>

                            <!-- Payment -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 font-medium capitalize">
                                ...
                                <div class="mt-1">
                                    ...
                                    @else
                                        <span
                                            class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">{{ ucfirst($order->payment_status) }}</span>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                ...
                                ">
                                    <option value="Pending" {{ strtolower($order->status) == 'pending' ? 'selected' : '' }}>
                                        Pending</option>
                                    <option value="Processing" {{ strtolower($order->status) == 'processing' ? 'selected' : '' }}>
                                        Processing</option>
                                    <option value="Shipped" {{ strtolower($order->status) == 'shipped' ? 'selected' : '' }}>
                                        Shipped</option>
                                    <option value="Delivered" {{ strtolower($order->status) == 'delivered' ? 'selected' : '' }}>
                                        Delivered</option>
                                    <option value="Cancelled" {{ strtolower($order->status) == 'cancelled' ? 'selected' : '' }}>
                                        Cancelled</option>
                                </select>
                            </td>
                            ...
                            <td>
                                <a href="{{ route('admin.view', $order->id) }}"
                                    class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-md transition-colors">View</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
