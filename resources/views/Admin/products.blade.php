...
@section('content')
    ...
    <div class="mb-6 bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        <form action="{{ route('admin.products') }}" method="GET" class="flex gap-4">
            <div class="flex-1">
                ...
                <div class="relative rounded-md shadow-sm">
                    ...
                    <input type="text" name="search" id="search" value="{{ request('search') }}"
                        class="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 sm:text-sm border-gray-300 rounded-md p-2 border"
            ...
            @if(request('search'))
                <a href="{{ route('admin.products') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 border border-transparent rounded-lg font-medium text-sm text-gray-700 shadow-sm hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                    Clear
                </a>
            @endif
        </form>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex-shrink-0 w-12 h-12">
                                    <img class="w-12 h-12 rounded-md object-cover border border-gray-200"
                                        src="{{ filter_var($product->gallery, FILTER_VALIDATE_URL) ? $product->gallery : asset($product->gallery) }}"
                                        alt="{{ $product->name }}">
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-500 max-w-xs truncate" title="{{ $product->description }}">
                                    {{ Str::limit($product->description, 50) }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                <a href="{{ route('admin.products.edit', $product->id) }}"
                                    class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 px-3 py-1.5 rounded-md hover:bg-indigo-100 transition-colors inline-block">Edit</a>
                                <a href="{{ route('admin.products.delete', $product->id) }}"
                                    class="text-red-600 hover:text-red-900 bg-red-50 px-3 py-1.5 rounded-md hover:bg-red-100 transition-colors inline-block"
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-10 whitespace-nowrap text-sm text-gray-500 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <button
                                        class="mt-4 px-4 py-2 text-sm font-medium text-indigo-600 bg-indigo-50 border border-transparent rounded-lg hover:bg-indigo-100"
                                        onclick="window.location.href='{{ route('admin.products.add') }}'">
                                        Add Product
                                    </button>
                                </div>
                            </td>
